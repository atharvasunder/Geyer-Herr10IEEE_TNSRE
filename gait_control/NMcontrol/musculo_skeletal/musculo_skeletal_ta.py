...
class musculo_skeletal_ta(BaseMusculoSkeletal):
    def __init__(self, interface_name, state_names, state_units, leg_name='leg_1'):
        ...
        self.p = config.MUSCULO_SKELETAL_DICTIONARY["TA"]
        self.l_slack = config.MUSCLE_DICTIONARY['TA']['l_slack']
        self.l_opt = config.MUSCLE_DICTIONARY['TA']['l_opt']

        self.ta_ankle_lever = None

    def update_lmtc(self, rbs_states):
        """
        Returns updated l_mtc for tibialis anterior (ankle only). 
    ...
    def get_lever_arms(self, rbs_states):
        """
        Returns updated lever arm for TA (hip, knee, ankle).
        """
        ankle_angle = rbs_states[self.leg_name]['q']['ankle']  # [rad]

        self.ta_ankle_lever = self._joint_lever_arm(
            joint_angle_rad=ankle_angle,
            r0=self.p["RADIUS"],
            joint_type="ankle",
            phi_max=self.p["PHI_MAX"]
        ...
        self.set_states()
        return [0.0, 0.0, self.ta_ankle_lever]

    def set_states(self):
        self.states[:] = (0.0, 0.0, self.ta_ankle_lever)

    def get_torque_contributions(self, F_mtc):
        """
        TA contributes NEGATIVE ankle torques.
        """
        ankle_torque = -F_mtc * self.ta_ankle_lever
